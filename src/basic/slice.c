...
static Status slice_create_output(const Tensor input, int *begin,
  ...
  int *out_dims_temp =
    aitisa_default_cpu_allocator()->raw_alloc(sizeof(*out_dims_temp)*in_ndim);
  if(!out_dims_temp){
  ...
  status = aitisa_create(dtype, device, layout_type, out_dims, out_ndim, &new_tensor);
  *output = new_tensor;
  return status;
}

static Status slice_check_parameters(const Tensor input, int *begin,
                                     int *size, int *step){
  int64_t in_ndim = aitisa_tensor_ndim(input);
...
static Status slice_template(const Tensor input, int *begin,
  ...
  int64_t *index_recorder =
    aitisa_default_cpu_allocator()->raw_alloc(sizeof(*index_recorder)*in_ndim);
  if(!index_recorder){
    return STATUS_ALLOC_FAILED;
  }
  for(int i=0; i<in_ndim; i++){
  ...
  int64_t *offset_recorder =
    aitisa_default_cpu_allocator()->raw_alloc(sizeof(*offset_recorder)*in_ndim);
  if(!offset_recorder){
    return STATUS_ALLOC_FAILED;
  }
...
Status aitisa_slice(const Tensor input, int *begin,
                    int *size, int *step,
                    Tensor *output){
  // make sure parameters are correct
  CHECK_STATUS(
    slice_check_parameters(input, begin, size, step));
  // create output
  CHECK_STATUS(
    slice_create_output(input, begin, size, step, output));
  // implement slice
  Status status;
